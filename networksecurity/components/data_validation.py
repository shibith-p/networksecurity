...
class DataValidation:
    ...
    def validate_numeric_columns(self, dataframe:pd.DataFrame) -> bool:
        try:
            return dataframe.select_dtypes(include=[float, int]).shape[1] == dataframe.shape[1]
        except Exception as e:
            raise NetworkSecurityException(e, sys)
        
    def detect_dataset_drift(self, base_df, curren_df, threshold=0.05)->bool:
        try:
            status = True
            report = {}
            is_found = False
    ...
    def initiate_data_validation(self) -> DataIngestionArtifact:
        try:
            ...
            status = self.detect_dataset_drift(base_df=train_dataframe, curren_df=test_dataframe)
